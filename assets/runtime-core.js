(function () {
  if (window.LargeGraphsRuntimeCore) {
    return;
  }
  ...
  function runtimeApi(root) {
    return root && root.__largeGraphsJlRuntimeApi ? root.__largeGraphsJlRuntimeApi : null;
  }
  ...
  function cleanup(root) {
    ...
    root.__largeGraphsJlRuntimeApi = null;
    root.__largeGraphsJlSigma = null;
    root.__largeGraphsJlPixi = null;
  ...
  function updateControls(root) {
    ...
    const api = runtimeApi(root);
    const enabled = Boolean(api && typeof api.hasContent === "function" && api.hasContent(root) && !root.__largeGraphsJlPaused);
    const lockEnabled = Boolean(api && typeof api.isReady === "function" && api.isReady(root) && !root.__largeGraphsJlPaused);
  ...
  function toggleCameraLock(root) {
    if (!root || !runtimeApi(root) || root.__largeGraphsJlPaused) {
      return;
    }
  ...
  async function fitView(root) {
    const api = runtimeApi(root);
    if (!api || typeof api.fitView !== "function") {
      return null;
    }
    return api.fitView(root);
  }
  ...
  function showPausedPreview(root, message) {
    ...
    button.addEventListener("click", function () {
      if (typeof root.__largeGraphsJlReactivate === "function") {
        void root.__largeGraphsJlReactivate(root);
      }
    });
  ...
  function updateHighlight(root, nodeId) {
    ...
    if (api && typeof api.updateHighlight === "function") {
      api.updateHighlight(root, selectedNode);
    }
  }
  ...
  function hoverNode(root, nodeId, pointer) {
    root.__largeGraphsJlHoveredNode = nodeId ? String(nodeId) : null;
    if (nodeId) {
      showTooltip(root, nodeId, pointer);
      dispatchInteractionEvent(root, interactionDetail(root, "hover", nodeId));
    }
  }

  function leaveNode(root, nodeId) {
    if (!root.__largeGraphsJlHoveredNode && !nodeId) {
      return;
    }
    root.__largeGraphsJlHoveredNode = null;
    hideTooltip(root);
    dispatchInteractionEvent(root, interactionDetail(root, "leave", nodeId || null));
  }

  function toggleSelection(root, nodeId, pointer) {
    if (!interactionEnabled(root, "enableSelection", true)) {
      return;
    }

    const nextSelected = root.__largeGraphsJlSelectedNode === String(nodeId) ? null : String(nodeId);
    updateHighlight(root, nextSelected);
    if (nextSelected) {
      showTooltip(root, nextSelected, pointer);
  ...
  function prepareRoot(rootId, loadingText) {
    ...
    status.textContent = loadingText || "Loading graph runtime…";
    ...
    const payload = parsePayload(root);
    if (!payload) {
      status.textContent = "Unable to parse graph payload";
      return null;
    }
    ...
    return { root, stage, status, payload, renderToken };
  }

  function isContextStale(context) {
    return !context || !context.root.isConnected || context.root.__largeGraphsJlRenderToken !== context.renderToken;
  }

  function finalizeRender(context, api) {
    if (!context || !api) {
      return null;
    }
    context.root.__largeGraphsJlRuntimeApi = api;
    context.root.__largeGraphsJlReactivate = typeof api.reactivate === "function" ? api.reactivate : null;
    ensureInteractionBridge(context.root);
    installCleanup(context.root);
    rememberActiveRoot(context.root);
    enforceActiveLimit(context.root);
    applyCameraLock(context.root);
    updateControls(context.root);
    return api;
  }

  function failRender(context, title, error) {
    if (!context) {
      return null;
    }
    context.status.textContent = title || "Graph runtime failed to load";
    ...
    context.stage.appendChild(details);
    applyCameraLock(context.root);
    updateControls(context.root);
    return null;
  }
  ...
  function renderOfflineFallback(context, reason, error) {
    if (!context) {
      return null;
    }
    ...
    note.textContent = reason || "Offline fallback";
    note.style.cssText = "position:absolute;right:12px;bottom:10px;font:600 11px/1.2 ui-sans-serif,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;color:#334155;background:rgba(255,255,255,0.88);padding:4px 8px;border-radius:999px;border:1px solid rgba(148,163,184,0.35);";
    stage.appendChild(note);

    if (error) {
      const details = document.createElement("pre");
      details.textContent = String(error);
      details.style.cssText = "position:absolute;left:12px;right:12px;bottom:40px;max-height:32%;overflow:auto;margin:0;padding:12px;background:rgba(255,255,255,0.92);border:1px solid #d1d5db;font:12px monospace;color:#991b1b;";
      stage.appendChild(details);
    }

    const context2d = canvas.getContext("2d");
    if (!context2d) {
      return failRender(context, "Offline fallback failed", error || "Canvas 2D context unavailable");
    }
    ...
    function draw() {
      ...
      context2d.fillStyle = payload.config?.background || "#ffffff";
      ...
      for (const edge of edges) {
        ...
        context2d.strokeStyle = edge.color || "#94a3b8";
        context2d.lineWidth = clamp(Number(edge.size) || 1, 0.5, 6);
      ...
      for (const node of nodes) {
        ...
        context2d.beginPath();
        context2d.arc(p.x, p.y, radius, 0, 2 * Math.PI);
        context2d.fillStyle = node.color || "#2563eb";
        context2d.fill();
      }
    }
    ...
    root.__largeGraphsJlRuntimeApi = api;
    applyCameraLock(root);
    updateControls(root);
    return api;
  }

  window.LargeGraphsRuntimeCore = {
    buildNeighborhoods,
    cleanup,
    clearSelection,
    dimmedEdgeColor,
